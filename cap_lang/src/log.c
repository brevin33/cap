...
void log_error_ast(Ast *ast, const char *format, ...) {
    ...
    log_error_va_list(start_token.file_index, start_token.start_char, end_token.end_char, format, args);
    va_end(args);
}
...
void log_error_token(Token *token, const char *format, ...) {
    va_list args;
    va_start(args, format);
    log_error_va_list(token->file_index, token->start_char, token->end_char, format, args);
    va_end(args);
}
...
void log_error(u32 file_id, u32 start_char, u32 end_char, const char *format, ...) {
    va_list args;
    va_start(args, format);
    log_error_va_list(file_id, start_char, end_char, format, args);
    va_end(args);
}

void log_error_va_list(u32 file_id, u32 start_char, u32 end_char, const char *format, va_list args) {
