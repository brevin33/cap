// This file was modified for CAP to follow project allocation pattern
...
// This is CAP adding this as I removed all free and changed malloc to alloc
#include "stdint.h"
...
static _WDIR* _wopendir(const wchar_t* dirname) {
    ...
    _WDIR* dirp = (_WDIR*)alloc(sizeof(struct _WDIR));
    if (!dirp) return NULL;
    ...
    dirp->patt = (wchar_t*)alloc(sizeof(wchar_t) * n + 16);
...
static int _wreaddir_r(_WDIR* dirp, struct _wdirent* entry, struct _wdirent** result) {
    ...
    if ((attr & FILE_ATTRIBUTE_DEVICE) != 0) entry->d_type = DT_CHR;
    else if ((attr & FILE_ATTRIBUTE_REPARSE_POINT) != 0) entry->d_type = DT_LNK;
    else if ((attr & FILE_ATTRIBUTE_DIRECTORY) != 0) entry->d_type = DT_DIR;
...
static void _wrewinddir(_WDIR* dirp) {
    ...
    if (!dirp || dirp->handle == INVALID_HANDLE_VALUE || !dirp->patt) return;
...
static DIR* opendir(const char* dirname) {
    ...
    struct DIR* dirp = (DIR*)alloc(sizeof(struct DIR));
    if (!dirp) return NULL;
...
static void _wseekdir(_WDIR* dirp, long loc) {
    ...
    if (dirp->handle == INVALID_HANDLE_VALUE) goto exit_failure;
    ...
    FindClose(dirp->handle);
    if (!dirent_first(dirp)) goto exit_failure;
...
static int scandir(const char* dirname, struct dirent*** namelist, int (*filter)(const struct dirent*),
    ...
    while (1) {
        ...
        if (!tmp) {
            tmp = (struct dirent*)alloc(sizeof(struct dirent));
